...
//! Manages user authentication: Windows 2000 style CTRL+ALT+DEL
//! login dialog with navy blue background, flat 3D dialog box,
...
fn addDefaultUser() void {
    ...
    u.username_len = setStr(&u.username, "Administrator");
...
pub fn dismissWelcome() void {
    if (login_state == .welcome) {
        login_state = .ctrl_alt_del;
    }
}

pub fn pressCtrlAltDel() void {
    if (login_state == .ctrl_alt_del) {
...
pub fn getTextColor() u32 {
    return theme.login_text;
}

pub fn getDialogColor() u32 {
