...
本仓库以 [ZirconOS](https://github.com/MobtgZhang/ZirconOS) 为参考，目标为 **NT 5.0（Windows 2000 时代）风格** 的混合内核与用户态体系。**不使用 GRUB**；引导仅通过自研 **ZirconOS Boot Manager（ZBM）**（UEFI 应用与 BIOS/MBR 链路）。
...
| P4 | LPC、I/O、驱动模型；FAT/NTFS 与 Win32 子系统（按需自上游移植） | 用户态与 shell |
| P5 | **win32k 骨架**：HAL 线性帧缓冲、GRE 子集（`ntgdi`）、桌面占位（`ntuser`）；x86_64 Multiboot2 帧缓冲标签 | `src/subsystems/win32/`、`src/hal/x86_64/framebuffer.zig` |
...
- **x86_64 + Multiboot2**：`src/arch/x86_64/boot.zig` 解析帧缓冲；`src/subsystems/win32/gre_early.zig` 初始化帧缓冲并调用 `desktop_session.bootstrapFromBootInfo()`（内部 `ntuser` + WM_PAINT 泵）。
...
- **桌面对象**：`subsystems/win32/winsta.zig`（WinSta0\\Default + OB 句柄）、`ntuser.zig`（HWND、按窗口消息队列）、`gdi.zig`（主表面 FillRect）、`ntgdi.zig`（Windows 2000 风格任务栏/开始菜单/托盘时钟占位）。
- **Syscall**：`arch/x86_64/syscall.zig` 分发 `win32_syscalls.zig`（0x1000..0x1010）；`libs/ntdll.zig` / `libs/user32.zig` / `libs/gdi32.zig` 为编号与文档桩。
- **日志**：`src/rtl/klog.zig`。
...
- **后续**：自上游移植完整 Ring3 `user32`/`gdi32`/Explorer 与真实 CSRSS 进程。

## 5.1 多架构说明（win32k / GRE）
...
- **RISC-V / MIPS**：内核可构建；与 win32k 帧缓冲启动链未完全打通时行为见 `docs/NT5_REACTOS_MATRIX.md`。
